...
class PreciseTimer {
    ...
    init?(common com: CocoaCB) {
        common = com

        pthread_create(&thread, &threadAttr, entryC, MPVHelper.bridge(obj: self))
        if thread == nil {
            common.libmpv.sendWarning("Couldn't create pthread for high precision timer")
            return nil
        }
    ...
    func updatePolicy(periodSeconds: Double = 1 / 60.0) {
        ...
        if !isHighPrecision {
            common.libmpv.sendWarning("Couldn't create a high precision timer")
        }
    }
