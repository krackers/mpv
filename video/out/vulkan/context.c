                           ...
                           struct bstr name, const char **value)
{
    ...
    for (int i = 0; i < num; i++) {
        ...
        if (help) {
            mp_info(log, "  '%s' (GPU %d, PCI ID %x:%x)\n",
...
const struct m_sub_options vulkan_conf = {
    .opts = (const struct m_option[]) {
        ...
        {"vulkan-queue-count", OPT_INT(dev_opts.queue_count), M_RANGE(1, 8)},
        {"vulkan-async-transfer", OPT_BOOL( dev_opts.async_transfer)},
        {"vulkan-async-compute", OPT_BOOL( dev_opts.async_compute)},
...
bool ra_vk_ctx_init(struct ra_ctx *ctx, struct mpvk_ctx *vk,  struct ra_vk_ctx_params params,
                    VkPresentModeKHR preferred_mode)
{
    ...
    p->protoInfo = (VkSwapchainCreateInfoKHR) {
        ...
        .minImageCount = ctx->vo->opts->swapchain_depth + 1, // +1 for FB
...
bool ra_vk_ctx_resize(struct ra_ctx *ctx, int w, int h)
{
    struct priv *p = ctx->swapchain->priv;
    if (w == p->w && h == p->h)
        return true;

    struct ra *ra = ctx->swapchain->ctx->ra;
    struct mpvk_ctx *vk = p->vk;
...
static bool start_frame(struct ra_swapchain *sw, struct ra_fbo *out_fbo)
{
    ...
    for (int attempts = 0; attempts < 2; attempts++) {
        ...
        case VK_ERROR_OUT_OF_DATE_KHR: {
            ...
            if (!ra_vk_ctx_resize(sw->ctx, w, h))
                return false;
            continue;
        }
...
static void swap_buffers(struct ra_swapchain *sw)
{
    ...
    while (p->frames_in_flight >= sw->ctx->vo->opts->swapchain_depth)
